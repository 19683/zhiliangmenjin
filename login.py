...
class User(UserMixin):
    ...
    # @password.setter
    def register(self, password, project):
        """save user name, id, project and password hash to json file"""
        ...
        with open(PROFILE_FILE, 'r+') as f:
            try:
                ...
                profiles = {}
            profiles[self.username] = [self.password_hash, self.id, project]
            f.seek(0, 0)
    ...
    def get_project(self):
        """get user project from profile file."""
        if self.username is not None:
            try:
                with open(PROFILE_FILE) as f:
                    user_profiles = json.load(f)
                    if self.username in user_profiles:
                        return user_profiles[self.username][2]
            except IOError:
                pass
            except ValueError:
                pass
